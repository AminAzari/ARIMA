...
#The Dickey-Fuller test for stationary check
from statsmodels.tsa.stattools import adfuller
...
rolmean = ts.rolling(window=22,center=False).mean()
...
plt.ylabel("RAW Daily Bitcoin price")
plt.show()
...
print(ts_logtrsfed.head(20))
test_stationarity(ts_logtrsfed)
#Determing rolling statistics
rolmean = ts_logtrsfed.rolling(window=22,center=False).mean()
rolstd = ts_logtrsfed.rolling(window=12,center=False).std()
#Plot rolling statistics:
orig = plt.plot(ts_logtrsfed, color='blue',label='Original')
mean = plt.plot(rolmean, color='red', label='Rolling Mean')
std = plt.plot(rolstd, color='black', label = 'Rolling Std')
plt.legend(loc='best')
plt.title('Log of raw data: Rolling Mean & Standard Deviation')
plt.show(block=False)
plt.ylabel("Log of Daily Bitcoin price")
plt.show()
...
print(ts_diff_logtrans.head(20))
test_stationarity(ts_diff_logtrans)
#Determing rolling statistics
rolmean = ts_diff_logtrans.rolling(window=22,center=False).mean()
rolstd = ts_diff_logtrans.rolling(window=12,center=False).std()
#Plot rolling statistics:
orig = plt.plot(ts_diff_logtrans, color='blue',label='Original')
mean = plt.plot(rolmean, color='red', label='Rolling Mean')
std = plt.plot(rolstd, color='black', label = 'Rolling Std')
plt.legend(loc='best')
plt.title('Dif of Log of raw data: Rolling Mean & Standard Deviation')
plt.show(block=False)
plt.ylabel("Dif of Log of Daily Bitcoin price")
plt.show()
